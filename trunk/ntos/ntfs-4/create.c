...

{
	...
	// EDIT.WUSD [2012-1-17 11:38]
	// [SHIT] 进入	CriticalRegion， 可以阻止normal APC的执行

	// * [User APCs           ] run strictly in user mode and only when the current thread is in an alertable wait state.
	//     The operating system uses user APCs to implement mechanisms such as overlapped I/O and the QueueUserApc Win32 routine.
	// * [Normal kernel APCs  ] run in kernel mode at IRQL = PASSIVE_LEVEL. A normal kernel APC preempts all user-mode code,
	//     including user APCs. Normal kernel APCs are generally used by file systems and file-system filter drivers.
	// * [Special kernel APCs ] run in kernel mode at IRQL = APC_LEVEL. A special kernel APC preempts user-mode code and 
	//     kernel-mode code that executes at IRQL = PASSIVE_LEVEL, including both user APCs and normal kernel APCs. 
	//     The operating system uses special kernel APCs to handle operations such as I/O request completion （IO的结束例程）.
