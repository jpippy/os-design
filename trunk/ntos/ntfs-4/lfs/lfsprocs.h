...
#include "..\sdk\inc\ntseapi.h"
#include "..\sdk\inc\ntapi.h"
#include "..\sdk\inc\fsrtl.h"
#include "..\sdk\inc\cache.h"
#include "..\sdk\inc\ex.h"
#include "..\sdk\inc\lfs.h"
#include "..\ntfs.h"

#include "lfs-nodetype.h"
#include "LfsDisk.h"
#include "LfsStruc.h"
#include "LfsData.h"
#include <string.h>
...
#define LfsAllocateLcb(NEW)     {                               \
    (*NEW) = FsRtlAllocatePool( PagedPool, sizeof( LfsLCB ));      \
    RtlZeroMemory( (*NEW), sizeof( LfsLCB ));                      \
    (*NEW)->NodeTypeCode = LFS_NTC_LCB;                         \
    (*NEW)->NodeByteSize = sizeof( LfsLCB );                       \
}

#define LfsInitializeLcb(LfsLCB,ID,MODE)                           \
    (LfsLCB)->ClientId = ID;                                       \
    (LfsLCB)->ContextMode = MODE

#define LfsDeallocateLcb(LfsLCB)   {                               \
    if ((LfsLCB)->RecordHeaderBcb != NULL) {                       \
        CcUnpinData( (LfsLCB)->RecordHeaderBcb );                  \
    }                                                           \
    if ((LfsLCB)->CurrentLogRecord != NULL                         \
        && (LfsLCB)->AuxilaryBuffer == TRUE) {                     \
        ExFreePool( (LfsLCB)->CurrentLogRecord );                  \
    }                                                           \
    ExFreePool( LfsLCB );                                          \
}
...
#define LfsValidateLcb(LfsLCB,LCH)                                         \
    if (LfsLCB == NULL                                                     \
        || (LfsLCB)->NodeTypeCode != LFS_NTC_LCB                           \
        || !LfsClientIdMatch( &(LfsLCB)->ClientId, &(LCH)->ClientId )) {   \
        ExRaiseStatus( STATUS_ACCESS_DENIED );                          \
    }
...
#ifndef LfsAdd2Ptr
#define LfsAdd2Ptr(_PTR, _INC, _CAST) ((_CAST)((PUCHAR)(_PTR) + (_INC)))
#endif  // LfsAdd2Ptr

#ifndef PtrOffset
#define PtrOffset(_BASE, _OFFSET) ((ULONG)((ULONG)(_OFFSET) - (ULONG)(_BASE)))
#endif  // PtrOffset
