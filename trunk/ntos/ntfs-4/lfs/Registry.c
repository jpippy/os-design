...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsInitializeLogFile:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Log File          -> %08lx\n", LogFile );
    LfsDebugTrace(  0, Dbg, "Maximum Clients   -> %04x\n", MaximumClients );
    LfsDebugTrace(  0, Dbg, "Log Page Size     -> %08lx\n", LogPageSize );
    LfsDebugTrace(  0, Dbg, "File Size (Low)   -> %08lx\n", FileSize.LowPart );
    LfsDebugTrace(  0, Dbg, "File Size (High)  -> %08lx\n", FileSize.HighPart );
    ...
    __try {
        ...
        __try {
            ...
            Lfcb->ClientArray = LfsAdd2Ptr( RestartArea, Lfcb->ClientArrayOffset, PLFS_CLIENT_RECORD );
            RestartArea = NULL;
            ...
            LfsDebugTrace( -1, Dbg, "LfsInitializeLogFile:  Exit\n", 0 );
        }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsOpenLogFile:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Log File          -> %08lx\n", LogFile );
    LfsDebugTrace(  0, Dbg, "Client Name       -> %08lx\n", &ClientName );
    LfsDebugTrace(  0, Dbg, "Maximum Clients   -> %04x\n", MaximumClients );
    LfsDebugTrace(  0, Dbg, "Log Page Size     -> %08lx\n", LogPageSize );
    LfsDebugTrace(  0, Dbg, "File Size (Low)   -> %08lx\n", FileSize.LowPart );
    LfsDebugTrace(  0, Dbg, "File Size (High)  -> %08lx\n", FileSize.HighPart );
    ...
    if (ClientName.Length > LFS_CLIENT_NAME_MAX) {
        ...
        LfsDebugTrace(  0, Dbg, "Illegal name length for client\n", 0 );
        LfsDebugTrace( -1, Dbg, "LfsOpenLogFile:  Exit\n", 0 );
        ...
        __try {
            ...
            while (Link != &LfsData.LfcbLinks) {
                ...
                if (ThisLfcb->FileObject == LogFile) {
                    ...
                    LfsDebugTrace( 0, Dbg, "Found matching log file\n", 0 );
                    break;
                }
            ...
            if (FlagOn( ThisLfcb->Flags, LFCB_LOG_FILE_CORRUPT )) {
                ...
                LfsDebugTrace( 0, Dbg, "The Lfcb is corrupt\n", 0 );
            ...
            while (ThisClient != LFS_NO_CLIENT) {
                ...
                if (ClientRecord->ClientNameLength == (ULONG) ClientName.Length
                    ...
                    LfsDebugTrace( 0, Dbg, "Matching client name found\n", 0 );
                    break;
                }
            ...
            if (ThisClient == LFS_NO_CLIENT) {
                ...
                if (ThisClient == LFS_NO_CLIENT) {
                    ...
                    LfsDebugTrace( 0, Dbg, "No free client records available\n", 0 );
            ...
            *LogHandle = (LFS_LOG_HANDLE) Lch;
            ...
            LfsDebugTrace(  0, Dbg, "Log Handle    -> %08ln\n", *LogHandle );
            LfsDebugTrace( -1, Dbg, "LfsOpenLogFile:  Exit\n", 0 );
        }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsCloseLogFile:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "LogHandle  ->  %08lx\n", LogHandle );
    ...
    __try {
        ...
        try_exit:  NOTHING;
            ...
            LfsDebugTrace( -1, Dbg, "LfsCloseLogFile:  Exit\n", 0 );
        }
    ...
    )
{
    ...
    LfsDebugTrace( +1, Dbg, "LfsReadLogFileInformation:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Log Handle    -> %08lx\n", LogHandle );
    LfsDebugTrace(  0, Dbg, "Buffer        -> %08lx\n", Buffer );
    LfsDebugTrace(  0, Dbg, "Length        -> %08lx\n", *Length );
    ...
    try_exit:  NOTHING;
        ...
        LfsDebugTrace( -1, Dbg, "LfsReadLogFileInformation:  Exit\n", 0 );
    }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsResetUndoTotal:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Log Handle        -> %08lx\n", LogHandle );
    LfsDebugTrace(  0, Dbg, "Number Records    -> %08lx\n", NumberRecords );
    LfsDebugTrace(  0, Dbg, "ResetTotal        -> %08lx\n", ResetTotal );
    ...
    __try {
        ...
        LfsDebugTrace( -1, Dbg, "LfsResetUndoTotal:  Exit\n", 0 );
    }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsRestartLogFile:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "LogFile    -> %08lx\n", LogFile );
    LfsDebugTrace(  0, Dbg, "Maximum Clients   -> %04x\n", MaximumClients );
    LfsDebugTrace(  0, Dbg, "Log Page Size     -> %08lx\n", LogPageSize );
    LfsDebugTrace(  0, Dbg, "File Size (Low)   -> %08lx\n", FileSize.LowPart );
    LfsDebugTrace(  0, Dbg, "File Size (High)  -> %08lx\n", FileSize.HighPart );
    LfsDebugTrace(  0, Dbg, "Pack Log           -> %04x\n", *LfsInfo );
    ...
    __try {
        ...
        if (LfsReadRestart( ThisLfcb,
            ...
            if (FirstValidPage) {
                ...
                DiskRestartArea = LfsAdd2Ptr( FirstRestartPage, FirstRestartPage->RestartOffset, PLFS_RESTART_AREA );
            }
            ...
            if (FirstChkdskWasRun
                ...
                ThisLfcb->ClientArray = LfsAdd2Ptr( RestartArea,
            ...
            //
                ...
                LfsDebugTrace( 0, Dbg, "Page size mismatch\n", 0 );
                ExRaiseStatus( STATUS_DISK_CORRUPT_ERROR );
            ...
            //
                ...
                LfsDebugTrace( 0, Dbg, "Log file has shrunk without clean shutdown\n", 0 );
                ExRaiseStatus( STATUS_DISK_CORRUPT_ERROR );
            ...
            //
                ...
                ThisLfcb->ClientArray = LfsAdd2Ptr( RestartArea, ThisLfcb->ClientArrayOffset, PLFS_CLIENT_RECORD );
                RestartArea = NULL;
        ...
        //
            ...
            LfsDebugTrace( 0, Dbg, "Log file has no restart area\n", 0 );
            ExRaiseStatus( STATUS_DISK_CORRUPT_ERROR );
        }
        ...
        LfsDebugTrace( -1, Dbg, "LfsRestartLogFile:  Exit\n", 0 );
    }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsNormalizeBasicLogFile:  Entered\n", 0 );
    ...
    *(PULONG)FileSize &= ~(PAGE_SIZE - 1);
    ...
    if ( *FileSize <= RestartPageBytes ) {
        ...
        LfsDebugTrace(  0, Dbg, "Log file is too small\n", 0 );
        LfsDebugTrace( -1, Dbg, "LfsValidateBasicLogFile:  Abnormal Exit\n", 0 );
    ...
    if (((PLARGE_INTEGER)&LogPages)->HighPart == 0
        ...
        LfsDebugTrace(  0, Dbg, "Not enough log pages -> %08lx\n", LogPages.LowPart );
        LfsDebugTrace( -1, Dbg, "LfsValidateBasicLogFile:  Abnormal Exit\n", 0 );
    ...
    LfsDebugTrace( -1, Dbg, "LfsNormalizeBasicLogFile:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsUpdateLfcbFromPgHeader:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb              -> %08lx\n", Lfcb );
    LfsDebugTrace(  0, Dbg, "System Page Size  -> %08lx\n", SystemPageSize );
    LfsDebugTrace(  0, Dbg, "Log Page Size     -> %08lx\n", LogPageSize );
    LfsDebugTrace(  0, Dbg, "Major Version     -> %04x\n", MajorVersion );
    LfsDebugTrace(  0, Dbg, "Minor Version     -> %04x\n", MinorVersion );
    ...
    LfsDebugTrace( -1, Dbg, "LfsUpdateLfcbFromPgHeader:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsUpdateLfcbFromNoRestart:  Entered\n", 0 );
    ...
    LfsDebugTrace( -1, Dbg, "LfsUpdateLfcbFromNoRestart:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsUpdateLfcbFromRestartArea:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb          -> %08lx\n", Lfcb );
    ...
    LfsFindOldestClientLsn( RestartArea,
                            LfsAdd2Ptr( RestartArea, Lfcb->ClientArrayOffset, PLFS_CLIENT_RECORD ),
                            &Lfcb->OldestLsn );
    ...
    LfsDebugTrace( -1, Dbg, "LfsUpdateLfcbFromRestartArea:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsUpdateRestartAreaFromLfcb:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb              -> %08lx\n", Lfcb );
    ...
    for (ClientIndex = 1,
         Client = LfsAdd2Ptr( RestartArea, Lfcb->ClientArrayOffset, PLFS_CLIENT_RECORD );
    ...
    LfsDebugTrace( -1, Dbg, "LfsUpdateRestartAreaFromLfcb:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsInitializeLogFilePriv:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb                  -> %08lx\n", Lfcb );
    LfsDebugTrace(  0, Dbg, "Force Restart         -> %04x\n", ForceRestartToDisk );
    LfsDebugTrace(  0, Dbg, "RestartAreaSize       -> %08lx\n", RestartAreaSize );
    LfsDebugTrace(  0, Dbg, "StartOffset (Low)     -> %08lx\n", StartOffsetForClear.LowPart );
    LfsDebugTrace(  0, Dbg, "StartOffset (High)    -> %08lx\n", StartOffsetForClear.HighPart );
    LfsDebugTrace(  0, Dbg, "Clear Log File        -> %04x\n", ClearLogFile );
    ...
    LfsDebugTrace( -1, Dbg, "LfsInitializeLogFilePriv:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsFindLastLsn:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb  -> %08lx\n", Lfcb );
    ...
    __try {
        ...
        if (!ValidFile) {
            ...
            if (NT_SUCCESS( Status )
                ...
                if (LfsCheckSubsequentLogPage( Lfcb,
                    ...
                    LfsDebugTrace( 0, Dbg, "Log file is fatally flawed\n", 0 );
                    ExRaiseStatus( STATUS_DISK_CORRUPT_ERROR );
                }
            }
        ...
        LfsDebugTrace( -1, Dbg, "LfsFindLastLsn:  Exit\n", 0 );
    }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsCheckSubsequentLogPage:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb                  -> %08lx\n", Lfcb );
    LfsDebugTrace(  0, Dbg, "RecordPageHeader      -> %08lx\n", RecordPageHeader );
    LfsDebugTrace(  0, Dbg, "LogFileOffset (Low)   -> %08lx\n", LogFileOffset.LowPart );
    LfsDebugTrace(  0, Dbg, "LogFileOffset (High)  -> %08lx\n", LogFileOffset.HighPart );
    LfsDebugTrace(  0, Dbg, "SequenceNumber (Low)  -> %08lx\n", SequenceNumber.LowPart );
    LfsDebugTrace(  0, Dbg, "SequenceNumber (High) -> %08lx\n", SequenceNumber.HighPart );
    ...
    if (*((PULONG) RecordPageHeader->MultiSectorHeader.Signature) == LFS_SIGNATURE_UNINITIALIZED_ULONG
        ...
        LfsDebugTrace( -1, Dbg, "LfsCheckSubsequentLogPage:  Exit -> %08x\n", FALSE );
        return FALSE;
    }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsRemoveClientFromList:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Client Array  -> %08lx\n", ClientArray );
    LfsDebugTrace(  0, Dbg, "Client Record -> %08lx\n", ClientRecord );
    LfsDebugTrace(  0, Dbg, "List Head     -> %08lx\n", ListHead );
    ...
    if (ClientRecord->PrevClient == LFS_NO_CLIENT) {
        ...
        LfsDebugTrace( 0, Dbg, "Element is first element in the list\n", 0 );
    ...
    LfsDebugTrace( -1, Dbg, "LfsRemoveClientFromList:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsAddClientToList:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Client Array  -> %08lx\n", ClientArray );
    LfsDebugTrace(  0, Dbg, "Client Index  -> %04x\n", ClientIndex );
    LfsDebugTrace(  0, Dbg, "List Head     -> %08lx\n", ListHead );
    ...
    *ListHead = ClientIndex;
    ...
    LfsDebugTrace( -1, Dbg, "LfsAddClientToList:  Exit\n", 0 );
