...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsPinReadLogPage:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb              -> %08lx\n", Lfcb );
    LfsDebugTrace(  0, Dbg, "FileOffset (Low)  -> %08lx\n", FileOffset.HighPart );
    LfsDebugTrace(  0, Dbg, "FileOffset (High) -> %08lx\n", FileOffset.LowPart );
    LfsDebugTrace(  0, Dbg, "Length            -> %08lx\n", Length );
    LfsDebugTrace(  0, Dbg, "PinData           -> %04x\n", PinData );
    LfsDebugTrace(  0, Dbg, "AllowErrors       -> %08x\n", AllowErrors );
    LfsDebugTrace(  0, Dbg, "IgnoreUsaErrors   -> %04x\n", IgnoreUsaErrors );
        ...
        *UsaError = FALSE;
        ...
        if (!NT_SUCCESS( Status )) {
            ...
            if (!AllowErrors) {
                ...
                LfsDebugTrace( 0, Dbg, "Read on log page failed -> %08lx\n", Status );
        ...
        //
            ...
            if (!IgnoreUsaErrors) {
                ...
                LfsDebugTrace( 0, Dbg, "Usa error on log page\n", 0 );
        ...
        LfsDebugTrace(  0, Dbg, "Bcb       -> %08lx\n", *Bcb );

        LfsDebugTrace( -1, Dbg, "LfsPinOrMapData:  Exit -> %08lx\n", Status );
    }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsPinOrMapLogRecordHeader:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb       -> %08lx\n", Lfcb );
    ...
    *RecordHeader = LfsAdd2Ptr( LogPageHeader, PageOffset, PLFS_RECORD_HEADER );

    LfsDebugTrace(  0, Dbg, "Record Header -> %08lx\n", *RecordHeader );
    LfsDebugTrace(  0, Dbg, "Bcb           -> %08lx\n", *Bcb );

    LfsDebugTrace( -1, Dbg, "LfsPinOrMapLogRecordHeader:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsCopyReadLogRecord:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb           -> %08lx\n", Lfcb );
    LfsDebugTrace(  0, Dbg, "RecordHeader   -> %08lx\n", RecordHeader );
    LfsDebugTrace(  0, Dbg, "Buffer         -> %08lx\n", Buffer );
    ...
    __try {
        ...
        while (TRUE) {
            ...
            RtlCopyMemory( Buffer,
                           LfsAdd2Ptr( PageHeader, LogPageOffset, PVOID ),
                           RemainingPageBytes );
            ...
            Buffer = LfsAdd2Ptr( Buffer, RemainingPageBytes, PVOID );
        }
        ...
        LfsDebugTrace( -1, Dbg, "LfsCopyReadLogRecord:  Exit\n", 0 );
    }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsFlushLfcb:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb          -> %08lx\n", Lfcb );
            ...
            if (LfsRestart) {
                ...
                if (NT_SUCCESS( Status )) {
                    ...
                    *Signature = LFS_SIGNATURE_RESTART_PAGE_ULONG;
                    ...
                    RtlCopyMemory( LfsAdd2Ptr( RestartPage, Lfcb->RestartDataOffset, PVOID ),
            ...
            //
                ...
                while (TRUE) {
                    ...
                    if (UseTailCopy) {
                        ...
                        SeqNumber = LfsAdd2Ptr( RecordPageHeader,
    ...
    try_exit:  NOTHING;
        ...
        LfsDebugTrace( -1, Dbg, "LfsFlushLfcb:  Exit\n", 0 );
    }
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsReadRestart:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb   -> %08lx\n", Lfcb );
    ...
    *LogPacked = FALSE;
    ...
    __try {
                ...
                if (Signature == LFS_SIGNATURE_MODIFIED_ULONG
                    ...
                    *RestartPageOffset = FileOffset;
                    ...
                    if (LfsIsRestartPageHeaderValid( FileOffset,
                        ...
                        RestartArea = LfsAdd2Ptr( ThisPage,
        ...
        LfsDebugTrace(  0, Dbg, "RestartPageAddress (Low)  -> %08lx\n", RestartPageAddress->LowPart );
        LfsDebugTrace(  0, Dbg, "RestartPageAddress (High) -> %08lx\n", RestartPageAddress->HighPart );
        LfsDebugTrace(  0, Dbg, "FirstRestartPage          -> %08lx\n", *FirstRestartPage );
        LfsDebugTrace( -1, Dbg, "LfsReadRestart:  Exit\n", 0 );
    }
...

{
    ...
    RestartArea = LfsAdd2Ptr( PageHeader, PageHeader->RestartOffset, PLFS_RESTART_AREA );
...

{
    ...
    if (LogPacked) {
        ...
        ClientArray = LfsAdd2Ptr( RestartArea,
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsFindFirstIo:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb          -> %08lx\n", Lfcb );
    ...
    *NextLbcb = NULL;
    ...
    LfsDebugTrace(  0, Dbg, "File Offset (Low)     -> %08lx\n", FileOffset->LowPart );
    LfsDebugTrace(  0, Dbg, "File Offset (High)    -> %08lx\n", FileOffset->HighPart );
    LfsDebugTrace(  0, Dbg, "Contains Last Entry   -> %08x\n", *ContainsLastEntry );
    LfsDebugTrace(  0, Dbg, "LfsRestart            -> %08x\n", *LfsRestart );
    LfsDebugTrace(  0, Dbg, "IoBlocks              -> %08lx\n", *IoBlocks );
    LfsDebugTrace( -1, Dbg, "LfsFindFirstIo:  Exit\n", 0 );
