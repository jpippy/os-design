...

{
    ...
    *(PULONG)FinalOffset += (DataLength + PageOffset);
    ...
    LfsDebugTrace(  0, Dbg, "FinalOffset (Low)     ->  %08lx\n", LogPageFileOffset.LowPart );
    LfsDebugTrace(  0, Dbg, "FinalOffset (High)    ->  %08lx\n", LogPageFileOffset.HighPart );
    LfsDebugTrace( -1, Dbg, "LfsLsnFinalOffset:  Exit\n", 0 );
...

{
    ...
    LfsDebugTrace( +1, Dbg, "LfsFindNextLsn:  Entered\n", 0 );
    LfsDebugTrace(  0, Dbg, "Lfcb          -> %08lx\n", Lfcb );
    ...
    __try {
        ...
        LfsDebugTrace(  0, Dbg, "Lsn (Low)     -> %08lx\n", Lsn->LowPart );
        LfsDebugTrace(  0, Dbg, "Lsn (High)    -> %08lx\n", Lsn->HighPart );
        LfsDebugTrace( -1, Dbg, "LfsFindNextLsn:  Exit -> %08x\n", FoundNextLsn );
    }
