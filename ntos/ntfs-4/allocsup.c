    ...
    while (FirstBit <= LastBit) {
        ...
        if (!RtlCheckBit( NewBitMap, FirstBit )) {
            ...
            if (((Vcb->TotalReserved + (Vcb->TotalReserved / 0x100) +
                 (llOne << Scb->CompressionUnitShift)) >= Vcb->FreeClusters) &&
            ...
            if (FlagOn(Scb->ScbState, SCB_STATE_WRITE_ACCESS_SEEN)) {
                Vcb->TotalReserved += llOne << Scb->CompressionUnitShift;
            }
        }
        FirstBit += 1;
    }
    ...
    while (FirstBit <= LastBit) {
        ...
        if (RtlCheckBit( BitMap, FirstBit )) {
            ...
            if (FlagOn(Scb->ScbState, SCB_STATE_WRITE_ACCESS_SEEN)) {
                LONGLONG llOne_1 = 1;
                LONGLONG llOne_2 = 1;
                ASSERT(Vcb->TotalReserved >= (llOne_1  << Scb->CompressionUnitShift));
                Vcb->TotalReserved -= llOne_2 << Scb->CompressionUnitShift;
            }
        }
        FirstBit += 1;
    }
