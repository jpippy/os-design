...

{
	...
	int aaa = 0;
	if(Irp->Tail.Overlay.OriginalFileObject->FileName.Length == 0x4c &&
		Irp->Tail.Overlay.OriginalFileObject->FileName.Buffer[1] == 'W')
	{
		aaa = 1;
	}
...

        ...
        if (NonCachedIo) {
            ...
            if ((((ULONG)StartingVbo) & (SectorSize - 1)
                ...
                //
                //  Perform the actual IO. Notice that if ScbAcquired is TRUE,
				//  then setting NtfsIoContext's Resource. -- Add by WangBin
                //
