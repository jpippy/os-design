    ...
    __try {
        ...
        for (Vcn = StartingVcn; Vcn <= EndingVcn; Vcn = Vcn + ClusterCount) {
            ...
            if (Lcn != UNUSED_LCN) {
                ...
                } __except (((GetExceptionCode() == STATUS_INSUFFICIENT_RESOURCES) &&
                           (IrpContext != NULL) &&
...

{
	...
	UNREFERENCED_PARAMETER(ThisVcn);
...

{
    ...
    __try {
		...
		UNREFERENCED_PARAMETER(BitmapClusters);
...
// implemented in file ./lfs/Registry.c
VOID
