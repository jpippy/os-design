...

{
    ...
    if (Length != 0) {
		typedef VOID (__stdcall * PFN_FsRtlIncrementCcFastReadWait)( VOID ); 
		PFN_FsRtlIncrementCcFastReadWait pfnIncreReadWait = NULL;
		UNICODE_STRING sIncrementCcRead = { 0 };
		RtlInitUnicodeString(&sIncrementCcRead, L"FsRtlIncrementCcFastReadWait");
		pfnIncreReadWait = (PFN_FsRtlIncrementCcFastReadWait) 
			MmGetSystemRoutineAddress(&sIncrementCcRead);
		ASSERT(pfnIncreReadWait);
...
#else
		if (pfnIncreReadWait) { pfnIncreReadWait(); }
#endif
...
#else
				{
					typedef VOID (__stdcall * PFN_FsRtlIncrementCcFastReadNotPossible)(VOID); 

					PFN_FsRtlIncrementCcFastReadNotPossible pfn = NULL;
					UNICODE_STRING sFuncName = { 0 };
					RtlInitUnicodeString(&sFuncName, L"FsRtlIncrementCcFastReadNotPossible");
					pfn = (PFN_FsRtlIncrementCcFastReadNotPossible)MmGetSystemRoutineAddress(&sFuncName);
					if (pfn)
					{
						pfn();
					}
