    ...
    do {
        ...
        __try {
            ...
            if (IrpSp->MinorFunction == IRP_MN_MOUNT_VOLUME) {
                ...
                if ((pMyIrpSp->MinorFunction == IRP_MN_USER_FS_REQUEST) &&
                    ((pMyIrpSp->Parameters.FileSystemControl.FsControlCode == FSCTL_SET_COMPRESSION) ||
                     (pMyIrpSp->Parameters.FileSystemControl.FsControlCode == FSCTL_MOVE_FILE))) {

                    if (!Retry) {

                        PIO_STACK_LOCATION NextIrpSp = NULL;
                        PMyIO_STACK_LOCATION pMyNextIrpSp = NULL;
                        NextIrpSp = IoGetNextIrpStackLocation( Irp );

                        pMyNextIrpSp = (PMyIO_STACK_LOCATION)NextIrpSp;
                        pMyNextIrpSp->Parameters.FileSystemControl.OutputBufferLength = MAXULONG;
                        pMyNextIrpSp->Parameters.FileSystemControl.InputBufferLength = MAXULONG;
                    }
                }
    ...
    __try {
        ...
        while (TRUE) {
            ...
            if (Attribute->TypeCode == $VOLUME_INFORMATION) {
                ...
                if (VolumeInformation->MajorVersion != 3) {
...

{
	...
	IrpSp =(PMyIO_STACK_LOCATION)( (Irp)->Tail.Overlay.CurrentStackLocation );
...
{
    PMyIO_STACK_LOCATION IrpSp;
    PMyIO_STACK_LOCATION NextIrpSp;
    ...
    IrpSp = (PMyIO_STACK_LOCATION)( (Irp)->Tail.Overlay.CurrentStackLocation );
    NextIrpSp = (PMyIO_STACK_LOCATION)((Irp)->Tail.Overlay.CurrentStackLocation - 1 );
...

{
    ...
    // http://msdn.microsoft.com/en-us/library/ms810040.aspx 
...

{
    ...
    __try {
        ...
        if (FileReferenceNumber == 0) {
            ...
            } __except (NtfsReadMftExceptionFilter( IrpContext,
...

{
    ...
    IrpSp = (PMyIO_STACK_LOCATION)IoGetCurrentIrpStackLocation( Irp );
    NextIrpSp = (PMyIO_STACK_LOCATION)IoGetNextIrpStackLocation( Irp );
    FsControlCode = IrpSp->Parameters.FileSystemControl.FsControlCode;
